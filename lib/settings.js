  ...
  memory: {
    ...
    items: {
      backend: {
        ...
        default: 'hindsight',
        label: '记忆后端',
        // 2026-09-20 用户原话：「文字太多了 简化一点」—— 旧 hint 804 字、9 对 `**`、16 个反引号，
        // 是全 spec **唯一**带标记的一条，也是次长一条（221 字）的 3.6 倍。改写成 158 字纯散文：
        // 只留三个帮用户做决定的事实（三选一互斥 / 选 Midas 的条件性停用 / 要重启），
        // 操作细节（`- insert:`、`disabled: true`、`MIDAS_MCP_DB` 无默认、装法步骤）已由状态块与
        // `midasSetup` 引导在**恰当时机**渲染，这里不再重复一遍（重复 = 两份会漂的拷贝）。
        // ⚠️ 这里**刻意不写**"设置页会如实说未接通"这类兜底话（2026-09-20 删掉）：它是为了让
        // `memory-backend.test.mjs` 里一条"这几个字必须在文件里出现"的空转断言变绿而加的，用户看不到
        // 任何差别 —— 三个未就绪档各自怎么说，真源在 `lib/memory-backend.js` 的
        // `classifyMidasReadiness`（逐态给"没接通 + 此刻仍走 Hindsight"），设置页那三处文案是同一件事
        // 的第三个家。写进 hint 只会多一份会漂的拷贝。别再加回来。
        hint: '三种选择互斥，选一个就停掉另一个。Hindsight 按 token 计费；Midas 是本地 SQLite、不花 token，'
          + '但不做整会话摘要。选 Midas 会停掉 Hindsight，前提是 Midas 真的接上了（找得到二进制）；没接通就不关，'
          + '否则你两个后端一起失去。改这一项要重启 dsh web 才生效。',
