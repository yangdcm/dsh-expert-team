...
async function listRunsInWorkspace(ws) {
  ...
  let names0 = [];
  try { names0 = await readdir(root0); } catch { return []; }
  const out0 = [];
  for (const n of names0) {
    const st0 = await readJsonSafe(join(root0, n, 'STATE.json'));
    if (st0) {
      const tasks0 = taskList(await readJsonSafe(join(root0, n, 'TASKS.json')));
...
async function listSubagentStatusBySession(ctx, sid, knownIds) {
  ...
  if (root) {
    try {
      ...
      const runtime = rt || (ctx && ctx.subagents) || null;
      if (runtime && typeof runtime.listChildren === 'function') {
        rows = await runtime.listChildren(root);
      }
    } catch (e) { /* best-effort: roster renders un-dispatched */ }
  }
...
<div class="wrap">
  ...
  <div class="hint">本画布由 <code>/team canvas</code> 动态生成（数据源：${esc(dir)}）。任务/成员状态随各阶段推进更新；想刷新就再跑一次 <code>/team canvas</code>。${watch ? ' <b>── 当前为 live 模式：每 3 秒轮询 host 状态接口自动刷新。</b>' : ''}</div>
</div>
${watch ? watchScript(runId, cwd) : ''}
</body>
</html>`;
}
...
function watchScript(runId, cwd) {
...
async function createRun(ctx, invocation, cwd, mode) {
  ...
  if (pickedTier && !(mode.roles && mode.roles.length)) {
    const next = narrowedRoles(pickedTier, DEFAULT_ROLES, DEFAULT_ROLES);
...
export function apply(ctx, config) {
  ...
  if (ctx && typeof ctx.on === 'function') {
    ...
    try {
      ...
      //
      // 为什么：对真实会话做全量遥测审计（`qa-t03/token-*.mjs`）——
      ...
      //   运行时探针：`qa-t03/toolface-probe-{scope,restrict}.mjs`（用宿主真函数跑）。
      //
