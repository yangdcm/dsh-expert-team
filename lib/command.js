...
const ROLE_WARM_STATS = { warms: 0, errors: 0, warmMs: 0, noProgress: false, noProgressAt: 0, lastReads: 0, lastQueued: 0 };
/**
...
function warmSubRoles(ctx, subs, wfLabels, opts) {
  ...
  const deadlineMs = (opts && Number.isFinite(opts.deadlineMs)) ? opts.deadlineMs : ROLES_READ_DEADLINE_MS;
  const stats = { reads: 0, queued: 0, cut: '' };
  const p = new Promise((resolve, reject) => {
    setTimeout(() => {
      // 开跑那一刻才垫额度、才算期限（见函数头注释）。
      ROLE_READS_LEFT = Math.max(ROLE_READS_LEFT, maxReads);
      resolveSubRoles(ctx, subs, wfLabels, { maxReads, deadlineAt: Date.now() + deadlineMs, stats }).then(resolve, reject);
    }, delay);
  })
    .then((r) => {
      ...
      // "有活、有期限，却一条都没读成" = **真故障**（不是按设计的上限，也不是"刷新在飞"）。
            ...
            handler: async (req, res) => {
              ...
              try {
                ...
                if (want('people')) {
                  ...
                  // ② 该读了 ⇒ 交给后台（单飞、有界）。踢不出去（已有后台批在跑 / 队列空 / 队列里
                  ...
                  // `scopeCaps.roles.over`（= 超出上限的条数）就成了一句假话。
                  ...
                  const kicked = warmSubRoles(ctx, subs.slice(0, MAX_ROLE_SUBS), wfLabels, { maxReads: rolesAllowance, deadlineMs: ROLES_READ_DEADLINE_MS });
