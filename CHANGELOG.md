...
两处根因都是同一族病（本仓反复登记过的「承诺 ≠ 能力，且不告诉你」）：

- lead 此前**一个 shell 都没有** ⇒ 它把 `grep` / `md5` / `stat` 这类**诊断/校验**命令写成一段文字
  **交给用户**，让用户自己去终端里跑。本包的用户大多不会跑 shell ⇒ "agent 自己能做的事"变成了
  "用户的工作"，也违背本包的基本契约（agent 自己执行、把结果回给用户）。
- 12 个角色 persona 都写了"必须用简体中文"，而 **lead 自己的 persona 是纯英文默认值** ⇒ 用户只跟 lead
  说话，于是他看到的是英文。`SKILL.md` 里那句救不了：**技能是按需加载的**，没被 `skill` 工具读到就等于
  不存在。契约的权威只能是**常驻通道**（preset 的 persona 行）。

**本版有两处对所有人生效、无开关的行为变更**（见下），这也是它为什么是 1.4.x → **1.5.0** 的小版本号。

### 行为变更（两处，均无开关）

- **① lead 现在能执行白名单内的只读 shell**（`lib/lead-toolface.js` + `lib/command.js` 接线）。
  此前 lead 的 shell 被一并 deny；现在改为**保留 shell + 只读白名单门禁**：
  - **allow-list 而不是 deny-list**。表内命令（逐条核过）：`pwd` · `echo` · `ls` · `cat` · `head` ·
    `tail`（禁 `-f`/`-F`/`--follow`，只读诊断不该阻塞会话）· `wc` · `stat` · `md5`/`md5sum` · `grep` ·
    `sed`（仅只读形态，`w`/`W`/`e` 被拒）· `git`（**只许 `diff`/`status`/`log`**，并挡掉 `-c` / `-C` /
    `--output` / `--ext-diff` / `--textconv` / `--exec-path` / `--config-env` / `--upload-pack`）·
    `node`（**只许 `--version`/`-v`**）。**`find` 故意不在表里**（`-exec`/`-delete` 是写与执行入口）。
    **没见过的命令一律拒绝**（fail-closed）。
  - **含 shell 元字符整条拒绝、不做净化**：`;` `|` `&` `>` `<` 反引号 `$` 换行/NUL 命中即整条拒绝 ——
    "只看第一个词"不可靠（`grep x f; rm -rf y` 的首词完全合法）。`\` `*` `?` `[` `'` `"` `~` 有意**不拒**
    （转义/只读通配/引号/家目录展开，拒它们只会误伤正当诊断）。
  - **门禁没挂上就不放行**（`shellGuardAvailable`，fail-closed）：宁可 lead 没有 shell，也不能给它一个
    **无约束**的 shell。挂载点是插件级 `tools/pre-execute`（与 R1 归属门禁同一挂法），且只对 lead 生效
    （对象身份判据）—— **角色子代理（qa 跑测试、backend 跑构建）保留完整 shell**。
  - **拒绝理由可读、可行动**：写明"为什么拒绝 + 还能做什么 + 该怎么做"，并**明说不要把命令交给用户去
    终端里跑**（那正是本次要修的产品问题）。
  - 顺带把一直没被调用的 `artifactReadDecision` 接进读路径：`read`/`read_image` 仍按**路径**收窄到工件子树，
    与诊断按**命令种类**收窄是两条口径 —— 这是有意的取舍，已写进代码注释。
- **② 凡需用户做选择，一律走可点击选项**（lead persona 新增【交互】契约）：澄清提问、方案确认、二选一/多选、
  档位选择、授权申请**必须用 `ask_user_question`** 把候选项做成可点击选项（每项带 `label` + `description`，
  推荐项排第一并标注「推荐」），**不得**写成"回复 A 或 B"这类纯文本。对**所有人、所有会话**生效，无开关。

### 修复

- **lead 不再用英文回答**（用户报障）：lead 的 **persona**（唯一常驻提示通道）新增【语言】契约，与 12 条
  角色 persona 同一条；`SKILL.md` 里那句自此是**从属表述**。
- **12 条角色 persona** 的【语言】条款各追加一句（共 12 处）：**结构化返回值**（`report` / `summary` /
  `verdict` 等字段的自然语言内容）与**子代理列表里显示的最终消息**同样使用简体中文；枚举值（`pass` /
  `needs_revision` / `reject`）、字段名、`path` 等机器可读标识保持原样、不翻译。
- **设置页的 hint 不再把标记原样显示**（`client.js`）：hint 与原生 `title` 都先过 `plainText()` 再 `esc()`
  （与其它 10 处调用点同一套顺序）。面板没有 markdown 渲染器 ⇒ 服务端 hint 一旦带标记就原样显示给用户；
  现存 19 条 hint 里 18 条本就是纯文本，故这是**防御性收口**，不靠"作者记得别写 markdown"这条纪律兜底。

### 守门

- `preset-lint.test.mjs` 新增 **§⓪ persona 语言/交互契约**断言，且**排在宿主探针之前**（该节只读文本、
  不需要宿主 schema ⇒ CI 也会真跑；若放在探针之后就会"本机绿、CI 恒不跑"）。
- `lead-toolface.test.mjs` 新增白名单 / 门禁 / 拒绝理由断言；`settings-page.test.mjs` 新增 hint 转义断言。
- 变异目录**仍 172 条**（`M132` 的 `find`/`replace` 随 `planLeadToolFace` 重写迁到新写法，**语义一字不变**）。

### 升级步骤

- lead 的 persona 与只读白名单在**插件加载时**生效，**无需**额外动作；但**正在运行的旧会话不会热更新** ——
  **新开一个会话**（或重启 `dsh web`）后才吃到新的【交互】契约与只读 shell 能力。
- 角色子代理的完整 shell 不受影响；本版**没有**"关掉 lead 只读 shell"的开关。

## 1.4.0

**主题：记忆后端三选一收成真正的互斥 —— 对已经在用 `midas` 的人是一次行为变更**。本版把
`memory.backend` 的三档（`hindsight` / `midas` / `off`）从"只做加法"改成"两行都归设置值管"：
**这是本版唯一的行为变更，也是它为什么是 1.3.x → 1.4.0 的小版本号而非补丁号。**

- **已经在用 `midas` 的人**：下一次在设置页保存后端选择（或再点一次那项配置）、**并重启 `dsh web`**
  之后，Hindsight 会被**真的关掉**（`- id: hindsight` 那一行写成 `disabled: true`）。后果是实打实的：
  **不再有 Hindsight 的上下文注入，也不再走它那套带 token 成本的整会话摘要**。这是**预期**的结果
  —— 三选一的语义就是"要么 Hindsight、要么 Midas、要么都不用"，此前选 `midas` 时两个后端同时在跑。
  不想失去 Hindsight 的人，把选择改回 `Hindsight` 即可（那一行会被恢复）。变更会在**当次**回执里
  说明，**不静默**改你的 profile。
- **已经在用 `off` 的人**：除了 `hindsight: disabled: true`，那个 `- insert:` 包裹的 `mcp-midas` 行
  现在会被**一并移除**（此前它被留在原处 ⇒ MCP 服务照旧随 `dsh web` 启动、工具照旧注册在模型工具面上，
  而界面说"不会有任何记忆调用"）。**其余行、注释、空行逐字保留**。
- **用默认 `hindsight` 的人不受影响**：什么都不改的人，补丁里那一行不会被禁用，MCP 行该在的在、
  该移除的移除 —— 与升级前一致。
- **升级步骤**：更新之后**重启 `dsh web`**（profile 补丁只在加载 profile 时读），然后**重新保存一次
  后端设置**，让补丁被改写成与设置值一致。在此之前界面上的"实际状态"与你的选择可能不一致，
  `readMemoryBackendState()` 会把这种不一致如实报出来，而不是假装已经生效。
- **安全前提不变**：选 `midas` 时"关掉 Hindsight"只在**真的接上了**（找得到 Midas 二进制、那一行马上
  要被写进补丁）才发生；二进制没找到时**一个都不关**（否则用户会同时失去两个后端）。
...
### Midas 二期：三选一收成真正的互斥（2026-09-19）

**这是一次行为变更，先把话说在前面**：在这之前，选 `midas` 只是做加法 —— 它清掉 Hindsight 的
禁用位、再把 MCP 行写进去，**不动**别的。于是选它的人**两个后端同时在跑**：Hindsight 仍在注入
上下文、仍按 token 计费，而设置页只报了一个后端。用户对这一点的要求是明确的 —— 要么 Hindsight、
要么 Midas、要么都不用，**要真的三选一**。这一节就是把它做成真的。

**现在的语义（设置值是用户的声明，两行都归它管）**：

| 设置 | `- id: hindsight` 那一行 | `- insert:` 包裹的 `mcp-midas` 那一行 |
|---|---|---|
| `hindsight`（默认） | **启用**（我们写的规范行整行删掉，别人的行只把 `disabled` 改成 `false`） | **移除** |
| `midas`（二进制找得到 = 真的接上了） | **`disabled: true`** | 写入 / 替换（含绝对路径、显式 `node`、显式 `MIDAS_MCP_DB`） |
| `midas`（**二进制没找到**） | **保持启用**（一个都不关） | **不写**（写了就是"假接通"） |
| `off` | **`disabled: true`** | **移除** |

- **对已经选了 `midas` 的人**：下一次在设置页保存后端选择（或再点一次那项配置）时，插件会把
  `hindsight` 那一行写成 `disabled: true`；**重启 `dsh web` 之后**就**不再有 Hindsight 的上下文
  注入**，也不再产生它的 token 成本。变化会在**当次**回执里说明（不静默改你的 profile）；不想失去
  Hindsight 的人，把选择改回 `Hindsight` 即可（那一行会被恢复）。
- **`off` 现在连 `mcp-midas` 那一行一起删掉**（此前它只写 `hindsight: disabled: true`，
  **不碰**那个 `- insert:` 行 ⇒ Midas 的 MCP 服务照旧随 `dsh web` 启动、它的工具照旧注册在模型的
  工具面上，而界面却说"不会有任何记忆调用"——**那句话在那个格子里是假的**）。现在三档的出口自检
  都**同时**回读两件事（Hindsight 那一行的启用位 × MCP 行在不在），对不上就 `ok:false`、
  **一个字节都不落盘**。
- **安全不变式（这条最要紧）**：选 `midas` 时"关掉 Hindsight"是**有前提**的 —— 只有**真的接上了**
  （找得到 Midas 二进制，那一行马上要被写进补丁）才关。**二进制没找到时不关**：那时 MCP 行写不进去
  （写一条指向不存在命令的行就是"假接通"），若顺手把 Hindsight 也关掉，用户就**同时失去两个后端**
  —— 那是最坏的结果。所以未接通时**默认路径不变**：记忆仍走 Hindsight（设置页如实说"未接通 + 记忆
  此刻仍走 Hindsight、仍按 token 计费"，并给出首装引导）。这不是漏做，是**故意的**。
- **代价照旧明说**：Midas **不做整会话摘要**（设计取舍）。切到 Midas = 省掉 token 与把记忆留在本地，
  换来的是"跨项目知识页不再由 LLM 汇总整段会话"；需要那一项就选 Hindsight。
- **不是"没有行为变更"**：上面一期那节末尾"本版不改任何既有设置的默认值"说的是**默认值**（默认仍是
  `hindsight`，什么都不改的人确实不受影响），但**已经选了 `midas` / `off` 的人**下次保存会被改写补丁
  —— 所以整个 `Unreleased` **有**行为变更，那一句已在原处就地更正（没有删：它在一期落地时是真的）。
...
- **本版不改版本号、不新增依赖、不改任何既有设置的默认值。** ⚠️ 这句只在**一期**（上面这一节）的
  范围内成立；**整个 `Unreleased` 并非没有行为变更**：二期把三选一收成真正的互斥之后，已经选了
  `midas` / `off` 的人下次保存时补丁会被改写（逐条见下面「二期」那一节）。原句留着会被读成
  "整个 Unreleased 都没有行为变更" —— 那是假的，所以在这里**就地更正**（不删：它在一期落地时
  是真的，删掉等于把当时说过的话抹掉）。
