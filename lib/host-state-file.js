 ...
 *   注意：上面这段论证**只覆盖了 `$DSH_HOME` 这一类**（"工作区之外"），当时**没有覆盖另一种越界**：
 *     **另一个项目目录**。后者的围栏不是"绝对在根外"，而是**相对**的——同一个
 *     `<项目目录>/team/<run-id>/...` 在"项目目录 = DSH 启动目录"时合法、在两者不同时被拒。
 *     这条盲点正是 Issue #1：插件适配层丢掉了 per-call 授权参数 `sandboxPolicy`（宿主
 *     `writeText` 的第 5 参），宿主遂按部署根（`process.cwd()`）判定归属。**已在
 *     `lib/artifact-writer.js` / `lib/command.js` 修好**（做法与边界见
 *     `docs/专家团-Issue1-跨目录写入根因与修复方案.md` 6.1）。
 *     ⇒ 本模块的结论**不变**：宿主状态文件（`$DSH_HOME` 等绝对越界路径）仍然不能走
 *     `ctx.fs`，仍然走下面这套"临时文件 + fsync + rename"。上面那句"越界必被拒"只是**论据的
 *     覆盖面被 Issue #1 校准过**，不是要改这里的实现。
