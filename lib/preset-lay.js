 ...
 * }} facts
 * @returns {{state: string, action: string, stale: boolean, complete: boolean|null, reason: string, missingBeforeLay: string[]}}
 */
export function classifyLayTarget(facts = {}) {
  ...
  const missingBeforeLay = expected.filter((n) => !present.has(n));
  // 期望清单为空 ⇒ **判不出来**（complete = null），此时不许把它当"齐全"从而归成 foreign-complete。
  const complete = expected.length === 0 ? null : missingBeforeLay.length === 0;

  if (exists !== true) {
    return { state: 'absent', action: 'lay', stale: false, complete, reason: 'target-absent', missingBeforeLay };
  }
  ...
  if (identityMatch === true) {
    return { state: 'ours', action: 'lay', stale: true, complete, reason: 'identity-match-without-stamp', missingBeforeLay };
  }
  // ③ 身份**明确不是**我们的，且入口文件**齐全** ⇒ 这就是用户的定制：绝不覆盖。
  if (identityMatch === false && complete === true) {
    return { state: 'foreign-complete', action: 'skip-user', stale: false, complete: true, reason: 'not-ours-and-complete', missingBeforeLay: [] };
  }
 ...
 *          pluginVersion?: string, missingBeforeLay?: string[], replaced?: string|null, at?: number}} patch
 */
export function recordLayOutcome(kind, patch = {}) {
  ...
  STATUS[kind] = {
    ...
    missingBeforeLay: Array.isArray(patch.missingBeforeLay) ? patch.missingBeforeLay.slice() : [],
...
export function presetLayStatus(opts = {}) {
  const one = (rec) => (rec ? { ...rec, missingBeforeLay: Array.isArray(rec.missingBeforeLay) ? rec.missingBeforeLay.slice() : [] } : null);
 ...
 *          pluginVersion?: string, missingBeforeLay?: string[], at?: number}|null} rec
...
export function describeLayOutcome(rec) {
  ...
  if (state === 'partial-or-unknown') {
    const n = rec && Array.isArray(rec.missingBeforeLay) ? rec.missingBeforeLay.length : 0;
