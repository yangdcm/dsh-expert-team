  ...
  memory: {
    ...
    items: {
      backend: {
        ...
        default: 'hindsight',
        label: '记忆后端',
        hint: '三种选择：Hindsight（自托管服务端，写入/召回都按 token 计费，跨项目知识页最完整）；'
          ...
          + '不使用（真的停掉：往 dsh profile 的 cordis.patch.yml 写一行 `- id: hindsight` + `disabled: true`，'
          + '之后不会再有记忆写入/召回，也没有 token 成本）。'
          + '改这一项**需要重启 dsh web** 才生效（profile 补丁在加载 profile 时读取）—— 设置页会告诉你改了什么、要不要重启。',
        labels: { hindsight: 'Hindsight（自托管，按 token 计费）', midas: 'Midas（本地，零 LLM）', off: '不使用（真的关掉记忆）' },
      },
    },
  },
};
