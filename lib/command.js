...
// 为什么要有它：审计 10 个真实顶层会话 + 57 个角色会话后发现，输出 tokens 只占总量 0.24%、
...
async function scaffoldRun(cwd, mode) {
  ...
  const task = [
    ...
    '## 完成标准',
    '',
    '（clarify 结束时由 lead 填写客观验收条件；deliver 按它验收）',
    '',
    '## 禁止操作',
    '',
 ...
 *
 * 解析顺序：① `via==='create'` 的会话（可靠）→ ② 该 run 最早的一条记录（兜底，可疑）。
 * 返回值带 `ownerResolved` 明示②是**推断**，调用方据此决定要不要在界面上如实提示。
 */
function runOwnerSession(runId) {
  if (!runId) return { sid: '', ownerResolved: false };
  const rows = [];
  for (const [sid, v] of SESSION_RUNS) {
    if (v && v.runId === runId) rows.push({ sid, at: Number(v.at) || 0, via: v.via || '' });
  }
  if (!rows.length) return { sid: '', ownerResolved: false };
  const created = rows.filter((r) => r.via === 'create').sort((a, b) => a.at - b.at);
  if (created.length) return { sid: created[0].sid, ownerResolved: true };
  rows.sort((a, b) => a.at - b.at);
  return { sid: rows[0].sid, ownerResolved: false };
}
 ...
 *
 * 为什么需要：`SKILL.md §7.22` 要求 lead 每次派工把 `<agentId>:<role>` 追加进 `STATE.members`
 ...
 *   ① 有 wfLabel 的候选：仅当 `wfLabels[id].runId === runId`（缺失或不等都拒）；
...
function filterRunScopedSubs(subs, opts = {}) {
  ...
  for (const sub of (Array.isArray(subs) ? subs : [])) {
    ...
    if (wf) {
      // ① 会话级 label 必须用 runId 收窄到本 run
      if (runId && String(wf.runId || '') === runId) picked.push({ sub, at: Number(wf.startedAt) > 0 ? Number(wf.startedAt) : Number.MAX_SAFE_INTEGER });
      else reject(id);
      continue;
    }
...
/** 仅供测试：清空行备忘与在飞标记。 */
function _resetSubRowsMemo() { SUB_ROWS_MEMO.clear(); SUB_ROWS_INFLIGHT.clear(); }
  ...
  const p = Promise.resolve()
    ...
    .then((rows) => {
      ...
      if (arr.length === 0 && expectRows) {
        ...
        SUB_ROWS_MEMO.set(key, { at: Date.now(), rows: arr, provisional: false, retryMs: retryBase, emptyStreak: 0 });
      }
...
async function listSubagentStatusBySession(ctx, sid, knownIds, opts) {
  ...
  // 如实标 `warming`：只有"**期望的成员一个细节都没拿到**"才算"还没有有效基线"。
  //   · 为什么放在这里而不是预热返回处：已结束的成员**本来就可能不在 `listChildren` 里**
  //     （它们要走下面的 rescue 枚举），所以"预热返回空"≠"还没热"。等整条流水线跑完再判断，
  //     才不会在**数据其实完整**的响应上贴一个假标记。
  //   · 真被期限截断时 `deadline` 优先（`cut || 'warming'` 不会盖掉它）。
  //   · 它**不进** degraded（不是失败：成员不丢，"细节不可得"由 `subsPending` 表达，
  //     与 rolesPending 同一口径）。
  if (root && ids.length) {
    const memoNow = SUB_ROWS_MEMO.get(root);
    const covered = rows.some((r) => ids.includes(r.id));
    if (memoNow && memoNow.provisional && !covered) SUB_HEADER_STATS.cut = SUB_HEADER_STATS.cut || 'warming';
  }
...
const ROLE_WARM_STATS = { warms: 0, errors: 0, warmMs: 0 };
/**
...
/** 仅供测试：复位后台角色读批的标记与计数。 */
function _resetRoleWarm() { ROLE_WARM_RUNNING = false; ROLE_WARM_STATS.warms = 0; ROLE_WARM_STATS.errors = 0; ROLE_WARM_STATS.warmMs = 0; }
/** 本轮允许枚举吗（节流窗口）？ */
  ...
  try {
    ...
    if (q && typeof q.readSession === 'function') {
      ...
      for (const ev of (snap && (snap.events || snap.log)) || []) {
        ...
        if (!r) { r = { runId, name: '', started: 0, settled: 0, stopReason: '', childIds: [], startedAt: 0, endedAt: 0, settledSeq: {} }; runs.set(runId, r); }
        const kind = ev.type.slice('tool-workflow/'.length);
        const t = Number(ev.time) || 0;
        if (kind === 'run-start') { r.name = String(d.name || ''); if (t) r.startedAt = t; }
          ...
          if (cid) {
            ...
            if (!labels.has(cid)) labels.set(cid, { label: String(d.label || ''), phase: String(d.phase || ''), runId, seq: Number(d.seq) || 0, startedAt: t, settled: false });
          }
 ...
 * 真正读日志这件事挪到本函数：**单飞**（全进程同刻最多一个后台读批）、有界（沿用
 * `ROLES_READ_PER_BURST` / `ROLES_READ_MIN_INTERVAL_MS` / `ROLES_READ_DEADLINE_MS` 三个既有闸）。
 *
...
function warmSubRoles(ctx, subs, wfLabels, opts) {
  ...
  const maxReads = (opts && Number.isFinite(opts.maxReads)) ? opts.maxReads : 0;
  if (!(maxReads > 0)) return null;
  const deadlineAt = (opts && Number.isFinite(opts.deadlineAt)) ? opts.deadlineAt : Infinity;
  if (!ROLE_PENDING.length) return null;   // 队列空 ⇒ 没有可做的活，别标 warming（否则常亮）
  ROLE_WARM_RUNNING = true;
  ROLE_WARM_STATS.warms += 1;
  const t0 = Date.now();
  // 后台批不像请求那样有"每请求预算"可依赖（`ROLE_READS_LEFT` 会被并发的请求复位），
  // 所以这里把本批的额度显式垫到位 —— 用量仍由 `maxReads` 收口。
  ROLE_READS_LEFT = Math.max(ROLE_READS_LEFT, maxReads);
  const delay = (opts && opts.immediate) ? 0 : rolesWarmDelayMs();
  const p = new Promise((resolve, reject) => {
    setTimeout(() => { resolveSubRoles(ctx, subs, wfLabels, { maxReads, deadlineAt }).then(resolve, reject); }, delay);
  })
    .then((r) => {
      ROLE_WARM_STATS.warmMs = Date.now() - t0;
      // 无论读了几条，本批结束后都进入节流窗口：否则"读到 0 条"（例如读失败没写缓存）
      // 会让每个 tick 都再踢一次、把 warming 变成常亮标记。
      ROLES_READ_LAST_AT = Date.now();
      ROLE_WARM_RUNNING = false;
      return r;
    })
    .catch((e) => { ROLE_WARM_STATS.errors += 1; ROLES_READ_LAST_AT = Date.now(); ROLE_WARM_RUNNING = false; return null; });
  p.catch(() => {});
  return p;
}
...
export function apply(ctx, config) {
  ...
  if (ctx && typeof ctx.on === 'function') {
    ...
    try {
      ...
      //     而真实用户需求只占 2%~11%；输出 tokens 只占总量的 **0.34%**。
          ...
          const dispose = registerLocal({
            ...
            handler: async (req, res) => {
              ...
              try {
                ...
                //   · `degraded` = 本轮**真的少算了东西**（命中硬上限 / 软期限截断）⇒ 报警；
                ...
                // 归属三源：① run 自带的 ownerSession → ② session-runs 里 via='create' 的记录
                // → ③ 该 run 最早一条记录（**推断**，用 ownerResolved=false 如实标注）。
                ...
                if (want('people')) {
                  ...
                  await resolveSubRoles(ctx, subs, wfLabels, { maxReads: 0 });
                  if (subs.length > MAX_ROLE_SUBS) degraded.push('roles:' + (subs.length - MAX_ROLE_SUBS));
                  // ② 该读了 ⇒ 交给后台（单飞、有界）。踢不出去（已有后台批在跑/队列空）就不标 warming。
                  // ⚠️ 交给后台的是 **`subs.slice(0, MAX_ROLE_SUBS)`**（与旧请求路径同一口径）：
                  // `resolveSubRoles` 内部会 `syncRolePending(传进去的那个列表)` **重建队列**，
                  // 传全量就会把超出上限的 id 也排进队列 —— 那样它们**迟早会被解析**，而
                  // `degraded` 里的 `roles:N`（N = 超出上限的条数）就成了一句假话。
                  // 便宜来源（wfLabels/缓存）的遍历仍在**全量** subs 上做（免费，纯为显示），
                  // 所以"超出上限"只意味着"不做日志解析"，不意味着"角色一律不显示"。
                  const kicked = warmSubRoles(ctx, subs.slice(0, MAX_ROLE_SUBS), wfLabels, { maxReads: rolesAllowance, deadlineAt: Date.now() + ROLES_READ_DEADLINE_MS });
                ...
                if (want('people')) try {
                  if (membersFromState(sel.stateMembers).byRole.size) {
                    ...
                    const scoped = filterRunScopedSubs(subs, {
                      wfLabels,
                      runId: sel.runId,
                      ownerSession: sel.stateOwnerSession || peopleSid,
