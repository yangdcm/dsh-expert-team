 ...
 *   ② **原子写**：同目录临时文件 → `fsync` → `rename`；文件 0600、目录 0700；校验不过**不落盘**；
...
/** 配置文件权限（含 token ⇒ 只给属主读）。 */
export const HINDSIGHT_CONFIG_FILE_MODE = 0o600;
/** 配置目录权限（不存在时才创建）。 */
export const HINDSIGHT_CONFIG_DIR_MODE = 0o700;
...
export function planHindsightConfigWrite(existingText, body = {}) {
  ...
  if (rawExisting.trim() !== '') {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      return refuse('现有配置文件存在但**不是合法 JSON 对象** ⇒ 拒绝写入（盲目合并会把你原有的内容整段写没）。请先手工修正该文件。');
    }
    existing = parsed;
  }
  ...
  if (changed.length === 0) {
    notes.push('提交的内容与现有配置**完全一致** ⇒ 没有改动，**未写盘**（不假报成功）。');
  ...
  notes.push(needsRestart
    ? `改动了 ${restartKeys.map((k) => '`' + k + '`').join(' / ')} ⇒ **需要重启 \`dsh web\`** 才生效（Hindsight 在启动时读这两个键）。`
...
/**
 * **原子写**：同目录临时文件 → 写 → `fsync` → `chmod 0600` → `rename`。
 *
 * 为什么必须同目录：`rename` 只在**同一文件系统**内原子；临时文件放 `/tmp` 再 rename 会退化成"复制"，
 * 中途断电就是半个文件。目录也 fsync（best-effort：个别平台不支持，失败不影响原子性）。
 * @param {string} path - 目标配置文件路径。
 * @param {string} text - 完整内容。
 * @returns {Promise<{ok: true, mode: number, tmpCleaned: boolean}>}
 */
export async function writeHindsightConfigAtomic(path, text) {
  const dir = dirname(path);
  await mkdir(dir, { recursive: true, mode: HINDSIGHT_CONFIG_DIR_MODE });
  const tmp = join(dir, `.${basename(path)}.tmp-${process.pid}-${Date.now().toString(36)}`);
  let fh = null;
  try {
    fh = await open(tmp, 'w', HINDSIGHT_CONFIG_FILE_MODE);
    await fh.writeFile(text, 'utf8');
    await fh.sync();                                  // 内容先真正落盘，再换名
    await fh.close();
    fh = null;
    await chmod(tmp, HINDSIGHT_CONFIG_FILE_MODE);     // umask 不再有机会削权限
    await rename(tmp, path);                          // 同目录 rename ⇒ 原子替换
  } catch (e) {
  ...
  } catch { /* 平台不支持目录 fsync：原子性不受影响 */ }
  return { ok: true, mode: HINDSIGHT_CONFIG_FILE_MODE, tmpCleaned };
}
