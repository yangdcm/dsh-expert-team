...
- **89 个测试文件 + 163 条变异目录**：`npm run test:all` 零依赖、无需 `install`（CI 跑的就是它）。变异目录在 CI 里校验的是**形状**（id 唯一、每个变异体的 `find` 串在目标文件里恰好命中一次、条数与常量一致）；**变异体本身需手动注入**，**CI 目前不执行变异体** ⇒ 它是"防呆 + 防漂移"，**不是**"自动证明测试能抓错"。
