...
- 插件加载时把设置注册为宿主命名空间 `expert-team`（`ctx.inject(['settings'], …)` 作为优雅降级边界）：
  - **数据层已就位**：设置由宿主服务持有，随插件市场的备份/恢复走；schema 由
    ...
    ⚠️ **更正（2026-09-14 实机核验）**：本节先前写的"出现在 设置 → 插件 → 插件配置"**不成立** ——
    那张页面渲染的是「宿主服务的命名空间 ∩ 已注册卡片」的交集，我们只交付了前半；
    客户端分节（slot `settings.plugin.item`，键为该命名空间；`dsh.client.inject` 需含
    `@deepseek-ai/dsh-client-ui-settings`）尚未交付。实施要点见
    `docs/专家团-设置卡片-实施要点.md`。
