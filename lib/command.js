...
async function workflowEventIndex(ctx, sid) {
  ...
  if (hit && (Date.now() - hit.at) < WF_EVENT_TTL_MS) return hit;
 ...
 */
async function workflowChildMeta(ctx, sid) {
  const { labels } = await workflowEventIndex(ctx, sid);
  const out = {};
 ...
 */
async function workflowRuns(ctx, sid, subs) {
  const { runs } = await workflowEventIndex(ctx, sid);
