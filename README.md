...
|---|---|---|---|---|
| 各带人设 / `toolFilter` / `maxDepth: 1` | 含 1 道硬门 + 1 道确认门 | 含 149 条变异目录与多组棘轮 | `dependencies: {}` | 无 bundler、无 `prepare` 钩子 |
...
- **零运行时依赖、零 devDependencies、无构建步骤、无 `prepare`/`postinstall` 钩子。** 装完就是能跑的那份代码，
  没有"安装时执行未知脚本"这一层。
- **89 个测试文件 + 149 条变异目录。** `npm run test:all` 无需 `install` 即可跑（CI 跑的就是它）。
