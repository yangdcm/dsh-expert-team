...
export async function readMemoryBackendState({ settings, env = process.env, home, workspace, io, hindsight, midas, mcpClientInstalled, midasProbe } = {}) {
  ...
  // 实际生效的后端：**先按 `stored` 分叉**，因为"用户选的是谁"决定了另一条线的补丁值还算不算数。
  //   · `stored === 'midas'` ⇒ 这一档的 else 分支（`hindsightDisabled ? 'off' : 'hindsight'`）
  //     **一个字都不适用**：用户根本不在 Hindsight 上。Midas 未就绪 ⇒ 真正在走的**永远**是
  //     Hindsight（不可能是 `'off'`）—— Hindsight 那一行有没有被禁用**不改变这件事**，
  //     而 `midas` 那几档的文案自己就写着"记忆仍走 Hindsight"⇒ 若这里回 `'off'`，同一个对象里
  //     `effective` 与 `display` 就说了两句相反的话（这正是本字段要消灭的自相矛盾，只是镜像的那一格）。
  //     「Hindsight 行被禁用」这个细节由 `display` / `notes` / `hindsightDisabled` 表达，
  //     不由 `effective` 表达 —— 两个语义层，别混。
  //   · 其余两档（`hindsight` / `off`）**以补丁文件为准**（与 conflict 那档的文案一致）。
  //
