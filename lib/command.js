...
function membersFromState(members) {
  ...
  for (const item of Array.isArray(members) ? members : []) {
    ...
    if (/^[\w-]{16,}$/.test(a) || /-/.test(a)) {
      ...
      byRole.set(a, role); // reverse key for debugging/feed lookup
    } else {
...
const SUB_ROWS_MEMO = new Map();     // root -> { at, rows }
...
function warmSubRows(ctx, root) {
  ...
  const p = Promise.resolve()
    ...
    .then((rows) => {
      ...
      SUB_ROWS_MEMO.set(key, { at: Date.now(), rows: arr });
...
async function listSubagentStatusBySession(ctx, sid, knownIds, opts) {
  ...
  if (root) {
    ...
    const stale = !memo || (Date.now() - (Number(memo.at) || 0)) > SUBS_ENUM_MIN_INTERVAL_MS;
    if (allowEnum && stale) {
      const warm = warmSubRows(ctx, root);
      if (!warm) {
        // `subagents` 服务不可用（老宿主）：保持原行为，交给下面的 rescue 枚举（仍受节流+期限管）。
      } else if (memo) {
        // 过期只管**后台**刷新，本轮先用旧行（已结束子会话的行不可变；活行已在上面覆盖）。
    ...
    try {
      ctx.inject(['webServer'], (wctx) => {
        wctx.effect(() => {
            ...
            handler: async (req, res) => {
              ...
              try {
                ...
                json(200, Object.assign(
                  ...
                  prof ? { profile: Object.assign({}, prof.steps, {
                    rolesBudget: budget,
          ...
          // ── 1.3.18 一期：Hindsight 记忆后端的**只读诊断** ────────────────────────────
          ...
          //
          // 三条硬纪律（有测试盯着）：① **只读** —— 不写文件、不 POST；② **token 值绝不回显**（只给
          // "是否已配置"布尔，连长度/前后缀都不给）；③ 任何失败 **fail-open**（notes 里说清，绝不 500）。
          const disposeHindsight = registerLocal({
            kind: 'exact',
            path: '/plugins/dsh-expert-team/hindsight-config',
            methods: ['GET'],
            handler: withRoute(async ({ json, query }) => {
