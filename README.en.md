...
|---|---|---|---|---|
| own persona / `toolFilter` / `maxDepth: 1` | 1 hard gate + 1 approval gate | incl. a 140-entry mutation catalog and several ratchets | `dependencies: {}` | no bundler, no `prepare` hook |
...
- **89 test files plus a 140-entry mutation catalog.** `npm run test:all` needs no `install` (it is what CI runs).
...
- `… -> 403 <!doctype html>…网站防火墙…` ⇒ the server's **WAF blocked the POST** (it returned an **HTML firewall page**, not the API's JSON error) ⇒ check whether the WAF blocks `/v1/`, or allow-list this client's IP/UA. **This is not a token problem.**
