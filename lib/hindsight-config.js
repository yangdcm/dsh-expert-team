 ...
 *   ① `403` + HTML ⇒ WAF（真实案例：`网站防火墙` 页）；② 共享内存/磁盘；③ 其它 5xx；
...
export function classifyFailure(text) {
  ...
  if (/\b403\b/.test(s) && hasHtml) {
    return {
      ...
      hint: '服务端 WAF 拦下了请求 —— 返回的是「HTML 防火墙页」，不是 API 的 JSON 错误。请检查 WAF 是否拦了 /v1/ 的 POST，或把该客户端 IP/UA 加入白名单。这不是 token 问题。',
    };
  }
...
/**
 * 从 `diag.jsonl` 文本里取**最后一条失败记录**。
 *
 * 规则（照实说清，避免"两种零混成一个"）：
 *   · 只认**带非空 `error` 字符串**的记录；`event === 'inject_empty'`（召回为空）**不算失败** ——
 *     "召回为空"与"调用失败"是两件不同的事；
 *   · 坏行/半行/非 JSON 行一律跳过（日志是被追加写的，最后一行可能是半行）；
 *   · 只看**最后 `DIAG_TAIL_LINES` 行**。
 * @param {string} diagText
 * @returns {{at: string|null, event: string|null, summary: string, classification: string, hint: string}|null}
 */
export function pickLastFailure(diagText) {
  ...
  for (let i = from; i < lines.length; i += 1) {
    ...
    if (!raw || raw.charAt(0) !== '{') continue;         // 半行/空行：跳过
    let rec = null;
    try { rec = JSON.parse(raw); } catch { continue; }   // 坏行：跳过（不抛）
    if (!rec || typeof rec !== 'object' || Array.isArray(rec)) continue;
    const ev = rec.event == null ? null : String(rec.event);
    const isFailure = typeof rec.error === 'string' && rec.error.trim() !== '' && ev !== 'inject_empty';
    if (isFailure) last = { ts: rec.ts == null ? null : String(rec.ts), event: ev, raw: rec.error };
  }
  if (!last) return null;
  const { classification, hint } = classifyFailure(last.raw);
  return { at: last.ts, event: last.event, summary: summarizeError(last.raw), classification, hint };
}
...
export async function buildHindsightReport({
  ...
  try {
    lastFailure = pickLastFailure(await io.readText(diagPath));
    if (!lastFailure) notes.push('诊断日志里没有失败记录（或只有"召回为空"这类非失败事件）。');
  } catch {
    notes.push('未找到 Hindsight 诊断日志（还没有可读的失败记录）。');
  }
