...
import { join, dirname, basename, resolve as pathResolve, sep as pathSep } from 'node:path';
...
// 记忆后端三选一（hindsight / midas / off）：**设置值 → profile 补丁 → 实际生效状态**的单一真源。
// 为什么单独成模块：`off` 的落地点是 dsh profile 的补丁文件（工作区之外），其"最小保真编辑 + 拒绝瞎合并 +
// 幂等 + 只走受控写入口"这四条纪律值得被单测钉住；路由只负责取设置/调用/回执（见 lib/memory-backend.js 文件头）。
// 2026-09-19 一期起，`midas` 也真的接线了（写 `- insert:` 包裹的 mcp-midas 行）+ 五态就绪自检 + 首装引导；
// 本文件那边只做两件事：**用 loader 条目名**回答"dsh 的 MCP 客户端装没装"，以及把 `?probe=1` 透传下去
// （真的启动一次 Midas 做握手 —— 探测必须由用户点，与 Hindsight 的连通性探测同一口径）。
import {
  readMemoryBackendState, applyMemoryBackend, removeMidasRow, MEMORY_BACKENDS, MEMORY_SETTING_PATH,
  MIDAS_MCP_CLIENT_PACKAGE, MIDAS_INSTALL_COMMAND, probeMidasBinarySync,
...
// 2026-09-20 起多一件事：shell **不再一收了之**，而是保留但用**只读白名单门禁**守住
//（用户投诉：lead 没 shell ⇒ 把 grep/md5/stat 交给用户去终端里跑），同时把一直没被调用的
// `artifactReadDecision` 接进读路径。门禁本体 = `createLeadToolFaceGate`（注入式 IO，可单测）。
import { planLeadToolFace, shouldNarrowLeadToolFace, agentScopedToolNames, createLeadToolFaceGate } from './lead-toolface.js';
...
import { SETTINGS_GROUPS, SETTINGS_SPEC, defaultSettings, normalizeSettings, mergeSettings, settingsSchema, compilePolicy, policyBlockFrom, getSetting } from './settings.js';
...
// Issue #1：沙箱授权解析需要**当前会话的 Session 对象**（宿主 `sandboxPolicy.resolve({ session })`
// 读的是 `session?.header.cwd` 与 `session.id`；传 id 无效）。这里存**对象**而不是 id：
// `/team` 的处理器里 `invocation.agent.session` 就是宿主给的会话对象，直接留引用即可，
// 省掉一次 `sessions.get()` 反查（少一个"服务不存在/查不到"的失败面）。
// 与 `WRITER_IDENTITY` 同源、同一处更新 —— 不制造第二份会话真相。
...
/**
 * 目标路径是否落在某个工作区根之下（**只**比较 `path.resolve` 归一化后的形态）。
 *
 * 为什么归一化必须**赢**（实测缺陷，`..` 那一类）：`pathIsUnder('/proj', '/proj/../etc/passwd')`
 * 曾经返回 `true` —— 因为比较里带上了一对**未归一化的原始串**，而
 * `'/proj/../etc/passwd'.startsWith('/proj/')` 先命中就直接放行；可这个目标的 `path.resolve`
 * 是 `/etc/passwd`，**根本不在 `/proj` 之下**。这个 `true` 会让调用方（`sessionOwningTarget`）
 * 把目标判给一个**并不拥有它**的会话 ⇒ 围栏根被设成 `/proj` ⇒ 一笔越界写入被放行
 * （不是"漏判退回无授权"的安全方向，而是**真的放行**）。
 * `..`／`.` 让"字面前缀"与"真实落点"背离，只有归一化形态代表真实落点，故这里**整对删掉**了
 * 原始串比较 —— 不是把它挪到后面、也不是"两者取或"：取或就等于漏洞仍在。
 *
 * 为什么盘根判定要在归一化前也做一遍：宿主 `sandboxPolicy.resolve()` 内部的
 * `resolveWorkspaceRoot` 会 `resolve(canonicalPath(path))`，本函数拿到的是调用方拼出来的
 * 绝对串；`''` / `/`（以及 Windows 盘根 `C:`）**一律不算**：那样的根等于"谁都归它管"，
 * 比不授权更危险。归一化只可能让这类根变得更"像路径"，故两处都要判。
 *
 * `target === root` 或 `target` 以 `root + path.sep` 开头算"之下"（前缀陷阱：根 `/proj`
 * **不**包含 `/project/team/x`）。全程 try/catch，绝不抛。
 */
function pathIsUnder(root, target) {
  try {
    const strip = (s) => String(s).replace(/[\\/]+$/, '');
    const rawRoot = strip(String(root || ''));
    if (!rawRoot || rawRoot === '/' || /^[A-Za-z]:$/.test(rawRoot)) return false;
    const t = pathResolve(String(target || ''));
    const base = strip(pathResolve(String(root || '')));
    if (!base || base === '/' || /^[A-Za-z]:$/.test(base)) return false;
    return t === base || t.startsWith(base + pathSep);
  } catch { /* 任何异常都当"不归属" */ }
  return false;
}

/**
 * 本次写入的**归属会话**（按目标路径判定）—— Issue #1 修复的后续修正（独立评审发现）。
 *
 * 为什么要按目标判定：授权若只按"环境里最后见过的会话"（`CURRENT_SESSION`）解析，那些
 * **不属于该会话**的写入就会被判越界。实测三处：`GET /state` 浮层写的是**用户正在看的**
 * 那个 run（`join(sel.workspace, 'team', sel.runId, 'STATE.json')`，`sel.workspace` 来自请求）、
 * `/decide`·`/plan`·`/plan/approve`·`/plan/discard` 写 `body.workspace`、
 * `dispatchLedger` 写**执行者会话**的 cwd（`tools/post-execute`）。
 * 修复前围栏根是 DSH 启动目录，这些写入在"目标恰好也在启动目录下"时是通过的；改成按环境会话
 * 授权后，同一笔写入可能翻成 `FS_SANDBOX_DENIED` —— 那是本次修复**新引入的失败方向**
 * （`/state` 那处还被 `catch {}` 吞掉，连报错都看不到）。
 *
 * 判据只有一条：**目标落在哪个会话的 cwd 之下，就由哪个会话授权**（最长前缀 = 最具体）。
 * 候选（按"假设最少"排序）：
 *   ① `ctx.get('sessions').list()` 的活会话（纯内存、零持久化 IO）—— 覆盖"另一个会话正在看/在写
 *      自己的 run"这一支；`cwdFromSession` 用的 `sessions.get(id)` 与它同源（同一个活存储）；
 *   ② `CURRENT_SESSION`（本次调用留下的会话对象；它可能已不在活存储里，故必须单独作为候选）；
 *      顺带保留"只拿得到 id 就反查一次"的旧口径（`sessions.get(id)`；`list()` 不可用时的兜底）；
 *   ③ 都不是 ⇒ `null` ⇒ 调用方**不传授权凭据** ⇒ 宿主回落部署根 = **修复前行为**（无回归方向）。
 *
 * ⚠️ 这不是"把围栏放宽"：授权根仍是**某个会话的 cwd**，目标不在它之下照样被宿主拒
 * （`cordis-fs-port.test.mjs` ⑧d 的假阳性守卫就是这条）。
 *
 * ⚠️ Issue #1 后续（独立评审发现的第三个缺陷）：**cwd 取值口径必须与宿主一致**。
 * `sessionCwdOf` 有 **4 级回落**（`header.meta.cwd → header.cwd → session.meta.cwd → session.cwd`），
 * 而宿主 `sandboxPolicy.resolve()` **只读一个来源**：
 * `workspaceRoot: resolveWorkspaceRoot(session?.header.cwd ?? this.workspaceRoot)`。
 * 于是"靠回落级 cwd 命中而当选"的会话（例如只有顶层 `session.cwd`），交给宿主解析时会
 * **静默回落部署根** ⇒ 目标再被判越界、写入被拒 —— 与 Issue #1 的现象逐字相同，且没有信号
 * （解析器返回的还是一份"看起来有授权"的凭据）。
 * ⇒ 归属判定**只认宿主会采纳的来源**（`hostHonoredCwdOf`）：没有 `header.cwd` 的会话**不当选**
 * ——返回"没有凭据"，而不是一份宿主会忽略的凭据。
 * 代价与结论（刻意如此，不回退）：只有顶层 `session.cwd` 而没有 `header.cwd` 的会话
 * ⇒ 解析器返回 `undefined` ⇒ 宿主按部署根判定 = **修复前行为**（不阻断写入、也不新增失败方向）。
 * `sessionCwdOf` 的 4 级回落**原样保留**给其它既有调用方（`/team` 的 cwd 推导 `command.js:6430`；`cwdFromSession` 内的反查 `command.js:3314`）。
 */
function sessionOwningTarget(ctx, target) {
  try {
    if (!target) return null;
    let best = null;
    let bestLen = -1;
    for (const s of candidateSessions(ctx)) {
      const cwd = hostHonoredCwdOf(s);
      if (!cwd || !pathIsUnder(cwd, target)) continue;
      const len = String(cwd).replace(/[\\/]+$/, '').length; // 最长前缀 = 最贴近目标的那个工作区
...
/** 授权候选会话（去重；机会式 `ctx.get`，绝不抛）。顺序无关，最终由 `sessionOwningTarget` 取最长前缀。 */
function candidateSessions(ctx) {
  const out = [];
  const seen = new Set();
  const push = (s) => {
    if (!s || typeof s !== 'object') return;
    const id = String((s.header && s.header.id) || s.id || '');
    if (id) { if (seen.has(id)) return; seen.add(id); }
    out.push(s);
  };
  let sessions = null;
  try {
    sessions = ctx && typeof ctx.get === 'function' ? ctx.get('sessions') : null;
    if (sessions && typeof sessions.list === 'function') for (const s of sessions.list() || []) push(s);
  } catch { /* 拿不到活存储 ⇒ 只剩下面两支 */ }
  push(CURRENT_SESSION);
  try {
    // 旧口径的兜底：只拿得到 id 时反查一次（与 `cwdFromSession` 同一套 ctx 访问路径）。
    // 只在宿主有 `sessions.get` 而 `list()` 不可用/取不全时有意义；拿不到就算了。
    const sid = String((CURRENT_SESSION && (CURRENT_SESSION.id || (CURRENT_SESSION.header && CURRENT_SESSION.header.id))) || (WRITER_IDENTITY && WRITER_IDENTITY.sessionId) || '');
    if (sid && sessions && typeof sessions.get === 'function') push(sessions.get(sid));
  } catch { /* 反查失败 ⇒ 不影响其余候选 */ }
  return out;
}

/**
 * 构造**按目标路径授权**的沙箱策略解析器：`sp.resolve({session})` 的 `workspaceRoot` 取自
 * "目标所在工作区的**归属会话**"的 cwd（必须是会话**对象**，传 id 无效）。
 * 解析不出归属会话 / 任何异常 ⇒ `undefined`（= 不传凭据 = 修复前行为），**绝不抛**。
 *
 * `onEvent`（可选）：与 `createArtifactWriter({onEvent})` **同一个通道**（调用方传同一个
 * `writerEvents`）。`{type:'policy-resolve-error', target, error}` **由本解析器负责上报**：
 * 异常正是在**这里**被吞掉的 —— 适配层的同名 catch 因此永远看不见它，事件在实践中永不发出
 * （"降级不再静默"就成了空头承诺；独立评审判定为最严重缺陷）。
 * ⚠️ 必须写成 `return await sp.resolve(...)`：`return <被拒绝的 promise>` **不会**被本层的
 * `try/catch` 接住（async 函数里 return 出的 thenable 在 try 之外才被采纳）—— 少了 `await`，
 * 异步形态的解析失败会穿透到适配层，而**同步抛错**（宿主 `resolve()` 就是同步方法）仍被吞掉，
 * 于是"谁来观测"随调用形态而变。`await` 让两种形态都落进同一个 catch、同一份上报。
 * 适配层那层 catch 保留为**纵深防御**（万一调用方传入的不是本函数；重复上报无副作用）。
 * 上报**不改**降级方向：仍然返回 `undefined`、绝不 rethrow，本次写入照旧不被打断。
 */
function policyResolverForTargets(ctx, sp, onEvent = null) {
  return async (targetPath) => {
    try {
      const session = sessionOwningTarget(ctx, targetPath);
      return session ? await sp.resolve({ session }) : undefined;
    } catch (e) {
      if (typeof onEvent === 'function') {
        try { onEvent({ type: 'policy-resolve-error', target: targetPath, error: String((e && e.message) || e) }); } catch { /* 观测不该影响写入 */ }
      }
      return undefined;
    }
  };
...
function adoptHostFs(ctx) {
  try {
    ...
    // Issue #1 的授权服务：同样机会式获取（**不进 `inject`** —— 理由同上方的 ARTIFACT 声明处：
    // 注入一个宿主没有的服务会让整条 `/team` 挂不上）。拿不到它 ⇒ 不传第 5 参，行为同修复前。
    const sp = ctx && typeof ctx.get === 'function' ? ctx.get('sandboxPolicy') : null;
    if (hostFs && typeof hostFs.writeText === 'function' && typeof hostFs.resolve === 'function') {
      // 每次写入按**本次要写的目标路径**解析授权：宿主据此把 `workspaceRoot` 定为"目标所属会话"的
      // cwd，围栏才与"工件落在会话项目目录"一致 —— 这既修掉 Issue #1 的跨目录拒绝，又不会因为
      // 写入目标是**别的会话**的工作区（浮层 / `/decide` / 派工台账）而误判越界。
      // 第三个参数把本层的事件通道（`writerEvents` → `pushActivityEvent`）交给适配层，
      // 用于上报授权解析失败（降级不再静默）。
      // ⚠️ 同一条通道也要交给**解析器本身**：异常是在解析器内部被吞掉的（`sp.resolve` 抛错
      // ⇒ 它自己 catch 后返回 undefined），适配层那层 catch 永远看不到 ⇒ 不传就永远静默。
      const resolvePolicy = sp && typeof sp.resolve === 'function' ? policyResolverForTargets(ctx, sp, writerEvents) : null;
      ARTIFACT = createArtifactWriter({ fsPort: cordisFsPort(hostFs, resolvePolicy, writerEvents), onEvent: writerEvents, identity: () => WRITER_IDENTITY });
...
function pushActivityEvent(e) {
  ...
  if (e.type === 'policy-resolve-error') {
    // Issue #1 后续（独立评审发现）：适配层原先把这个异常**静默吞掉** ⇒ 授权解析失败会悄悄把围栏
    // 从「目标所属会话的 cwd」退回「宿主部署根」—— 一种只有写入开始被拒时才看得见的降级。
    // 降级本身保留（本次写入照旧尝试），但必须响亮：这里如实说出"本次按无授权处理"。
    console.warn(`[expert-team] 沙箱授权解析失败（本次按"无授权"处理 ⇒ 回落宿主部署根；写入目标 ${e.target || ''}）：${e.error || ''}`);
  }
}
...
const OPTIONAL_PLUGINS = [
  ...
  // ── Midas（2026-09-19 一期）────────────────────────────────────────────────
  // 它**不是 dsh 插件**，而是一个 npm 全局命令（`midas-mcp`，MCP stdio 服务）⇒ `loaderEntryNames`
  // 永远看不到它，`match` 也永远为假。所以这一项的 installed/usable 由 `probe` 自己给。
  //
  // ⚠️ **只在用户真的选了 `midas` 时才探、才提示**（`installed:null` ⇒ status 'unknown' ⇒ 不进提示）：
  // 默认后端是 Hindsight，对着所有用户每次加载劝装一个他用不上的东西 = 噪声，而本仓的教训是
  // "噪声的代价是所有告警一起被降权"（见上面 INSTALL_FAILURE_WARNED 的注释）。选了却没装 ⇒ 那时提示
  // 才是**对的时间、对的人**。
  //
  // ⚠️ 探测用 `probeMidasBinarySync()`（**同步、不 spawn 任何进程**）：这条自检跑在 apply 之后的有界
  // 延迟重探里，在里面 spawn 一个 `npm prefix -g` 会卡住事件循环。它只看"不 spawn 的那几层"，
  // 与设置页那条完整发现**共用同一份候选清单**（`midasBinaryCandidates`），不是第二份路径表。
  //
  // ⚠️ `probe(names, env)` 的**第二个入参**是给用例的注入口（与 `readMemoryBackendState` 的
  // `midas:` 夹具同一套纪律）：不注入时由调用方填 `process.env`，行为与从前逐字相同。
  // 为什么非有这个口子不可：这一项判定的就是"这台机器上装没装 midas-mcp"，而**这台机器上
  // 真的可能装着**（用户自己装的）⇒ 不注入就变成"测宿主"而不是"测代码"：同一条断言在开发机上
  // 红、在干净 CI 上绿，而且**没有环境变量能关掉它**（裸名字那几层候选与 PATH 无关也照样命中）。
  // 注入 env 只改"看哪些路径"，探测逻辑与分档判据一个字都没绕过。
  {
    id: 'midas',
    pkg: 'midas-memory-mcp',
    match: () => false,
    what: '本地零 LLM 记忆服务（本地 SQLite，写入/召回不花 token；不做整会话摘要）',
    install: MIDAS_INSTALL_COMMAND,
    probe: (names, env) => {
      // 没选 midas ⇒ 两个 null ⇒ 'unknown' ⇒ 不提示（也不假报"没装"）。
      let backend = null;
      try { backend = getSetting(currentSettings(), MEMORY_SETTING_PATH); } catch { backend = null; }
      if (backend !== 'midas') return { installed: null, usable: null };
      let found = null;
      try {
        const p = probeMidasBinarySync({ env: (env && typeof env === 'object') ? env : process.env });
        found = p.probeOk === true ? p.found === true : null;
      } catch { found = null; }
      if (found === null) return { installed: null, usable: null };
      if (!found) return { installed: false, usable: false };
      const clientOk = mcpClientInstalledFrom(names);
      // 二进制在、客户端不在 ⇒ installed-not-ready（**照纪律不给安装命令**：那句笼统提示由设置页的
      // 首装引导逐条给出 —— 那里才能说清"缺的是哪一步"，而不是在这里塞第二条命令）。
      return { installed: true, usable: clientOk === true };
    },
  },
];
...
/**
 * `@deepseek-ai/dsh-mcp-client` 装没装 —— **只认 loader 条目名**（与上面两条探测同一套事实来源）。
 *
 * 为什么必须问 loader 而不是查文件：MCP 客户端是一个**插件行**（补丁里那一行的 `name:` 指向它），
 * 而 dsh 的补丁机制在"包里没有这个依赖"时只会打一行 warn 并跳过 ⇒ 文件在不在不代表它被挂上了。
 * `null` = 探测不可用（**不是**"没装"）—— "两种零必须分得开"的同一套口径。
 * 单一实现、两个调用点（推荐插件自检 / memory-backend 路由），不在两处各写一遍判定。
 */
function mcpClientInstalledFrom(names) {
  if (!names) return null;
  return names.some((n) => n === MIDAS_MCP_CLIENT_PACKAGE || n.indexOf(`${MIDAS_MCP_CLIENT_PACKAGE}/`) === 0);
}
 ...
 * 自检推荐插件。**纯读、无副作用**。
 *
 * 第二个入参 `opts.env` 是**给带 `probe` 的条目**用的环境注入口（不传 ⇒ `process.env`，
 * 生产路径行为与从前逐字相同）。为什么要它：midas 那一项判定的是"这台**机器**上装没装
 * `midas-mcp`"，而用户自己装过之后，宿主机上有它 —— 那种情况下断言测的是宿主而不是代码
 * （见 `OPTIONAL_PLUGINS` 里 midas 那条的注释）。注入口只改"看哪些路径"，分档判据一个字不变。
 * @param {object} ctx
 * @param {object} [opts]
 * @param {object} [opts.env] - 供 `probe(names, env)` 用的环境变量（默认 `process.env`）。
 * @returns {{items:Array, needsAttention:Array, hint:string}}
 *   status: 'ready'（装了且可用）/ 'installed-not-ready'（装了但当下不可用）/ 'missing'（没装）
 *           / 'unknown'（探测不可用 ⇒ 不下结论）
 */
function detectOptionalPlugins(ctx, opts) {
  const probeEnv = (opts && opts.env && typeof opts.env === 'object') ? opts.env : process.env;
  const names = loaderEntryNames(ctx);
  const items = OPTIONAL_PLUGINS.map((spec) => {
    // 有的条目**loader 看不见**（Midas 是 npm 全局命令，不是 dsh 插件）⇒ 它自带 `probe` 给出
    // installed/usable；没有 `probe` 的走原来的两条通用路径。两者都只回答 true/false/**null**。
    let probed = null;
    if (typeof spec.probe === 'function') {
      try { probed = spec.probe(names, probeEnv) || null; } catch { probed = null; }
    }
    const installed = probed ? (probed.installed === undefined ? null : probed.installed) : (names ? names.some(spec.match) : null);
    const usable = probed
      ? (probed.usable === undefined ? null : probed.usable)
      : (spec.id === 'hindsight' ? hindsightToolReady(ctx) : (spec.id === 'cost-meter' ? costMeterReady(ctx) : null));
    let status;
    // 只有**确定**的结论才给确定的状态：
...
/**
 * 从一个**会话对象**读出它的工作区 cwd（`header.meta.cwd → header.cwd → meta.cwd → cwd`）。
 *
 * 为什么单独抽出来（Issue #1 附带修复）：写入路径手里**直接就有会话对象**
 * （`invocation.agent.session`），不该为了拿 cwd 再绕一次 `sessions.get()` 反查；
 * 而口径必须唯一（两处各抄一份 4 级回落 = 下次只改一处就漂移）。
 * 本函数只做取值、不做兜底：拿不到返回 `undefined`，由调用方决定回落。
 */
...
/**
 * 会话的 cwd —— **只取宿主真正会采纳的那一个来源**（`session.header.cwd`）。
 *
 * 与 `sessionCwdOf` 的 4 级回落**故意不同**，因为宿主 `sandboxPolicy.resolve()` 是
 * `workspaceRoot: resolveWorkspaceRoot(session?.header.cwd ?? this.workspaceRoot)`
 * —— 回落级 cwd（`header.meta.cwd` / `session.meta.cwd` / `session.cwd`）宿主**看不见**。
 * 用回落级 cwd 去"代表宿主授权"，等于把一份宿主会忽略的凭据交给调用方：
 * 围栏静默退回部署根、写入被拒且无任何信号（Issue #1 后续缺陷，见 `sessionOwningTarget` 注释）。
 * ⇒ 凡是**用来预测宿主行为**的地方（沙箱归属判定）都必须用这一份；
 *   纯展示 / 路径推导仍用 `sessionCwdOf`（4 级回落，为兼容既有调用方保留）。
 * 取不到返回 `undefined`：不猜、不兜底。
 */
function hostHonoredCwdOf(session) {
...
async function executeTeamCommand(ctx, invocation) {
  ...
  // Issue #1：同一处留下会话**对象**，供沙箱授权解析使用（`sandboxPolicy.resolve({session})`
  // 读的是 `session.header.cwd` 与 `session.id`，传 id 无效）。
  CURRENT_SESSION = invocation?.agent?.session ?? null;
  WRITER_IDENTITY = { sessionId: String(invocation?.agent?.session?.id || ''), runId: '', role: 'lead' };
  const c = parseTeamCommand(invocation.rawInput);
  // Issue #1 附带修复：写入路径的 cwd 与会话的真实项目目录**同源**。
  // 旧口径只读 `header.cwd`、缺失时静默回落 `process.cwd()`（= DSH 启动目录）；主因修好后，
  // 这层偏差会把「围栏拒绝」变形为「写到了错误的目录」，而两侧（沙箱判定 / 路径推导）本该同源。
  // 取法：① 会话对象自带的 cwd（权威、免反查）→ ② 既有权威取法 cwdFromSession（按 id 反查）
  //       → ③ **保留** process.cwd() 兜底（其它代码依赖命令在拿不到 cwd 时仍可用，不硬失败）。
  const cwd = sessionCwdOf(CURRENT_SESSION) ?? cwdFromSession(ctx, CURRENT_SESSION?.id) ?? process.cwd();
...
/**
 * **谁是 lead** —— 只读 shell 白名单 / 读路径边界的**唯一路由判据**（2026-09-20）。
 *
 * 为什么用 `WeakSet` 而不是"每次工具调用都去问 `agents.roots()` + `composedPreset`"：
 *   ① 门禁挂在**插件级** `tools/pre-execute` 上（与 R1 归属门禁同一挂法，那条已在生产里跑通），
 *      它会对**每一个** agent 的每一次工具调用触发 —— 判据必须 O(1) 且不抛；
 *   ② `WeakSet.has(agent)` 对任何输入都不抛（非对象一律 `false`），比字符串比较更不可能误伤；
 *   ③ **白名单在这里就是安全闸门**：角色子代理（qa 跑测试、backend 跑构建）必须保留**完整** shell，
 *      只靠"作用域过滤"不够 —— 多一道对象身份判据，误伤的路径就断在这里。
 * 生命周期：agent 由 agents 服务强引用，agent 销毁后本表条目由 GC 回收，无需手工清理。
 */
const LEAD_AGENTS = new WeakSet();
/**
 * 白名单门禁是否**确认挂上了**。
 *
 * 只有 `true` 才把 shell 从 deny 名单里挪出来（见 `planLeadToolFace` 的 fail-closed 口径）：
 * 挂不上门禁却放行 shell = 给 lead 一个**无约束**的 shell —— 那比"lead 没有 shell"危险得多。
 * 注册发生在插件装配期、`agent/created` 之前，所以这个标志在第一次判定时已经定型。
 */
let LEAD_GATE_READY = false;
/** lead 额外**可以读**的路径前缀：专家团自己的技能/模板目录（它的"工作说明书"），理由见接线处注释。 */
const leadReadExtraRoots = () => {
  try { return [SKILL_DIR, fileURLToPath(PRESET_SRC).replace(/[\\/]+$/, '')]; } catch { return []; }
};
...
export function apply(ctx, config) {
  ...
  if (ctx && typeof ctx.on === 'function') {
    ...
    try {
      ...
      // ── lead 只读 shell 白名单 + 读路径边界（2026-09-20 · 真实用户投诉的修复）──────────
      // 事故：lead 一个 shell 都没有 ⇒ 它把 `grep` / `md5` / `stat` 这类**诊断**命令写成文字交给
      //   **用户**去终端里跑；而本包的用户大多不会跑 shell。这等于把 agent 自己能做的事变成用户的工作。
      // 修法（用户选定的方案）：**保留 lead 的 shell，但用只读白名单守住**（判定纯函数见
      //   `lib/lead-toolface.js` 的 `shellWhitelistDecision`），并把一直**没有被调用**的
      //   `artifactReadDecision` 接进读路径（它此前是死代码 —— 判定写好了，没人调）。
      //
      // 为什么挂在**插件级** `tools/pre-execute`（而不是 `agent.ctx` 上）：与上面 R1 归属门禁同一挂法，
      //   那条已在生产里跑通；"只管 lead"由**对象身份判据**（`LEAD_AGENTS`）保证，而不是靠作用域过滤
      //   猜 —— 猜错的代价是把角色子代理的 bash 一起拦掉（qa 跑不了测试、backend 跑不了构建）。
      // 挂不上 ⇒ `LEAD_GATE_READY` 保持 false ⇒ `planLeadToolFace` 把 shell 一起 deny（fail-closed）。
      try {
        const leadGate = createLeadToolFaceGate({
          // 唯一路由判据：这个 agent 是不是 `agent/created` 里确认过的那一个（见 LEAD_AGENTS 的注释）。
          isLead: (agent) => LEAD_AGENTS.has(agent),
          // 读路径按**会话工作区**解析（与宿主的 `file_path` 解析基准一致），取不到 ⇒ undecidable ⇒ 放行。
          cwdFor: (exec) => cwdFromSession(ctx, exec?.agent?.session?.header?.id || exec?.agent?.session?.id),
          // 额外放行 lead 读**专家团自己的**技能/模板目录：skill 的 `references/*.md`（角色 prompt 模板、
          // 工作区 schema）是 lead 的"工作说明书"，技能包自己要求"按需加载"⇒ 把说明书挡在边界外会
          // 直接把 lead 的编排能力打掉。范围只有这两个包内目录，且**只放宽读**，不放宽写。
          extraRoots: leadReadExtraRoots(),
          onEvent: (type, payload) => {
            try { console.warn(`[expert-team] ${type}`, JSON.stringify(payload)); } catch { /* 观测失败不影响工具 */ }
          },
        });
        ctx.on('tools/pre-execute', leadGate);
        LEAD_GATE_READY = true;
      } catch { /* 挂不上 ⇒ 不设标志：shell 会被 deny（fail-closed），而不是变成无约束 */ }
      ...
      ctx.on('agent/created', async ({ agent }) => {
        try {
          ...
          // `shellGuardAvailable` 传的是**门禁真的挂上了没有**（装配期已定型）：
          //   挂上了 ⇒ shell 不进 deny、改由白名单门禁守（否则模型看不见 bash，白名单成死代码）；
          //   没挂上 ⇒ shell 留在 deny 名单里（fail-closed，绝不出现"无约束的 shell"）。
          const plan = planLeadToolFace({ platform: process.platform, knownNames, shellGuardAvailable: LEAD_GATE_READY });
          if (!plan.effective) {
          ...
          // ④ 登记"这个 agent 是 lead" ⇒ 只读 shell 白名单 / 读路径边界门禁才会对**它**生效。
          //    顺序要紧：登记必须发生在任何工具调用之前（本回调由 `agent/created` 在创建时刻触发，
          //    宿主自己的 `restrict` 也是在这个时序上做的）。
          LEAD_AGENTS.add(agent);
    ...
    try {
      ctx.inject(['webServer'], (wctx) => {
        wctx.effect(() => {
          ...
          // ⚠️ **一个刻意的例外（2026-09-17 加）**：`memory.backend` **不**由这条路由应用。
          // 它的 `off` 要把 dsh profile 的补丁文件（`cordis.patch.yml`）改成 `disabled: true`，而 profile
          // 是**加载时**读的 ⇒ 那个动作**确实需要重启 `dsh web`**。所以它走专用路由
          // `/plugins/dsh-expert-team/memory-backend`（由 `lib/memory-backend.js` 落地，并在回执里如实回
          // `needsRestart: true` + 原因）；这条路由只负责**存值**（存值本身不需要重启），但补丁里含这一项时
          // 会在 `note` 里**明说**"只存了值、没落地，要去专用路由"—— 绝不留下"无需重启"而让人以为已经生效。
          /**
           * 把一份设置补丁落进**两处存储**（宿主命名空间 + `$DSH_HOME/expert-team/settings.json`），
           * 并刷新进程内缓存。**单一实现、两个调用点**（`/settings` 与 `/memory-backend`）——
           * 两处各抄一遍必然分叉（本仓头号返工源）。
           * @returns {Promise<{ok: boolean, status: number, errors: string[], settings: object, viaHost: boolean}>}
           */
          const persistSettingsPatch = async (patch) => {
            const merged = mergeSettings(currentSettings(), patch);
            if (!merged.ok) return { ok: false, status: 400, errors: merged.errors, settings: merged.value, viaHost: false };
            ...
            try {
              ...
              return { ok: false, status: 500, errors: [`设置写入失败：${String((e && e.message) || e)}`], settings: currentSettings(), viaHost: false };
            }
            ...
            return { ok: true, status: 200, errors: [], settings: merged.value, viaHost: viaHost.ok };
          };
          ...
          const disposeSettings = registerLocal({
            ...
            handler: withRoute(async ({ req, res, json, body }) => {
              ...
              if (method === 'GET') {
                // ⚠️ 2026-09-20 修复「选了 Midas，重启 dsh web 后下拉又跳回 Hindsight」：这里曾经是
                // **唯一**绕开 `currentSettings()` 的设置读取口 —— 读的是 `SETTINGS_CACHE || loadSettingsSync()`，
                // 即**只读文件存储** `~/.dsh/expert-team/settings.json`。而 `memory.backend` 按设计
                // **从不写进文件存储**（它是 `enum`、宿主表达得了 ⇒ `persistSettingsPatch` 对它跳过文件存储），
                // 只落在**宿主命名空间** `~/.dsh/settings.yaml`。文件里压根没有 `memory` 这个键 ⇒
                // `normalizeSettings` 填上 spec 默认 `hindsight`。于是同一个进程、同一个键出现两个
                // **互斥**的答案：本路由答 `hindsight`，而 `/memory-backend` 答真值 `midas`
                // （后者用的是 `currentSettings()` —— 宿主值叠在文件值之上的**唯一读取口**）。
                // 用户看到的正是「同一个值，两个路由两种说法」。
                // 修法：settings 一律走 `currentSettings()`（与 `/memory-backend`、`compilePolicy` 等
                // **所有**消费者同一口径；宿主不可用时它内部同样退回文件值，行为与 1.1.x 完全一致）。
                // 只有 `repaired` 仍取自缓存 —— 那是**加载时**文件解析的修复记录，而 `currentSettings()`
                // 返回的是**裸设置对象**（宿主合并结果里没有"修复记录"这个概念），响应形状要保持不变。
                const cached = SETTINGS_CACHE || loadSettingsSync();
                const merged = currentSettings();
                json(200, { ok: true, settings: merged, schema: settingsSchema(), repaired: cached.repaired, path: settingsPath(), groups: SETTINGS_GROUPS });
                return;
              }
              // POST：补丁可以嵌套（{roster:{maxTasks:10}}）也可以扁平（{'roster.maxTasks':10}）
              const patch = body && typeof body === 'object' && body.patch ? body.patch : body;
              const written = await persistSettingsPatch(patch);
              if (!written.ok) {
                json(written.status, { ok: false, errors: written.errors, settings: written.settings });
                return;
              }
              // `needsRestart` **恒为 false**（2026-09-15 逐项查明；2026-09-17 复核例外后仍然成立）：
              // 这条路由写下去的值里，容量/轮次上限、档位门、振荡检测开关由上面
              // `reapplySettingsDerived()` 当场重算；`identity.*` 每次建 run 时经
              // `compilePolicy(currentSettings())` 现读；`SETTINGS_CACHE` 就在上一行刷新。
              // 此前按"补丁顶层键 ∈ {roster,gates}"置 true 并回一句"重启后生效"，那是**假话**：
              // 同一个补丁刚在上一行被重算过。字段保留是为了不破坏客户端契约，但它只能说真话。
              // ⚠️ 唯一"需要重启"的项是 `memory.backend`，但它**不由这条路由应用**（见上面的例外），
              // 所以恒 false 仍然成立；补丁里含它时，`note` 会明说"只存了值、没落地"。
              const needsRestart = false;
              const memoryInPatch = !!(patch && typeof patch === 'object'
                && (Object.prototype.hasOwnProperty.call(patch, 'memory.backend')
                  || (patch.memory && typeof patch.memory === 'object' && Object.prototype.hasOwnProperty.call(patch.memory, 'backend'))));
              json(200, {
                ok: true,
                settings: written.settings,
                needsRestart,
                settingsSource: written.viaHost ? 'host' : 'file',
                note: (written.viaHost
                  ...
                  + (memoryInPatch
                    ? ' ⚠️ 补丁里含 `memory.backend`：这条路由**只存值**，不会去改 dsh profile 的补丁文件 —— 要让选择真的落地（尤其是 `off`），请走 `/plugins/dsh-expert-team/memory-backend`。'
                    : ''),
              });
            }, { methods: ['GET', 'POST'] }),
          });
          ...
          // ── 记忆后端三选一（`hindsight` / `midas` / `off`）────────────────────────────
          // 为什么必须是**独立**一条路由（而不是塞进 `/settings`）：`off` 的落地点不在设置存储里，
          // 而在 dsh profile 的补丁文件（`$DSH_HOME/profiles/<profile>/cordis.patch.yml`）—— profile 在
          // **加载时**才读 ⇒ 这条路由是本插件**唯一**会回 `needsRestart: true` 的地方
          // （`/settings` 的 `needsRestart` 恒 false 只对"它自己写下去的值"成立，见那条路由的例外说明）。
          //
          // GET：回答"**到底现在走哪个后端**" —— 设置值是多少 / 补丁有没有真的禁用 Hindsight /
          //      两者是否一致 / 以及"你选了不使用"与"选了 Hindsight 但不通"的区别。前两个是用户的选择，
          //      最后一个是故障：**渲染成同一个灰掉的东西**会让人做出错误处置（本仓纪律：两种零分得开）。
          // POST `{backend}`：① 先落地补丁（`lib/memory-backend.js` → 受控入口原子写）；② 再存值
          //      （`persistSettingsPatch`：宿主 + 文件两处存储的**同一份实现**，不另抄一遍）；③ 回读一次
          //      真实状态。回执里**绝不含机密** —— 那个模块只回路径/计数/文案，从不回显文件内容。
          //
          // POST `{action:'remove-midas-row'}`（2026-09-19 加，治那个"关不干净"的缺陷）：
          //      选「不使用」只会禁用 `hindsight` 行，**不碰**那个 `- insert:` 包裹的 `mcp-midas` 行 ⇒
          //      Midas 的 MCP 服务照样启动、`mcp__midas__*` 工具照样注册，而界面原来说"不会有任何记忆
          //      调用"（假话）。这是用户显式要求"把那一行清掉"的动作。
          //      ⚠️ **刻意不新增路由**：`registerLocal(` / `methods` 的条数是 `routes-shared.test.mjs`
          //      钉死的棘轮（各 15），多一条路由就是直接红 —— 所以在**同一条**路径上按 `action` 分派。
          //      分派判据刻意选 `action` 而**不是**"`backend` 缺失"：后者会把"忘了带 backend"的写法
          //      也当成删除动作（一次误删用户补丁行的机会），而且 `{action:'x'}` 这种写法在本仓
          //      `/preset-lay` 已有先例（同一套约定，客户端不必学第二种）。**未知 action 一律 400**，
          //      不静默回落到"存设置值" —— 静默 no-op 是本仓最忌讳的形态。
          //      ⚠️ 这个动作**不**改设置值：一次动作只改它声明要改的那一处。
          const disposeMemoryBackend = registerLocal({
            kind: 'exact',
            path: '/plugins/dsh-expert-team/memory-backend',
            methods: ['GET', 'POST'],
            handler: withRoute(async ({ json, body, req, query }) => {
              const method = String((req && req.method) || 'GET').toUpperCase();
              // 只读诊断摘要（**不开启连通性探测** —— 探测必须由用户点，与 `/hindsight-config` 同口径）：
              // 只用来把"启用了但不通"和"启用了"分开。fail-open：读不到就 `null`（那时不做判断）。
              const hindsightSummary = async () => {
                try {
                  const rep = await buildHindsightReport({ env: process.env });
                  const lf = rep && rep.lastFailure;
                  const recovered = !!(lf && lf.sinceFailure && lf.sinceFailure.recovered === true);
                  return { exists: rep.exists === true, failing: !!(lf && !recovered) };
                } catch { return null; }
              };
              // Midas 的"差一步"判定要用**已装插件清单**（loader 条目名）。为什么在这里探、不在 apply 里探：
              // 那条线踩过的坑 —— `apply()` 当刻服务表还是空的，那时下结论就是假警报；请求时探才是事实。
              // `loaderEntryNames()` 拿不到 ⇒ `null` ⇒ 状态归 unknown（不假装知道）。
              const mcpClientInstalled = () => mcpClientInstalledFrom(loaderEntryNames(ctx));
              if (method === 'POST') {
                const action = String((body && body.action) || '').trim();
                if (action && action !== 'remove-midas-row') {
                  // 未知动作**不许**静默回落到"存设置值"：那会让一个写错 action 的客户端以为删掉了。
                  json(400, { ok: false, error: 'unsupported action', allowed: ['remove-midas-row'], action });
                  return;
                }
                if (action === 'remove-midas-row') {
                  const removed = await removeMidasRow({ env: process.env, workspace: process.cwd() });
                  if (!removed.ok) {
                    json(removed.status || 400, {
                      ok: false, path: removed.path, rowId: removed.rowId,
                      errors: removed.errors, notes: removed.notes,
                    });
                    return;
                  }
                  // 回读一次**真实状态**：删完之后的 `midasRowPresent` 必须已经是 false（两端对齐），
                  // 客户端也就不用自己把"我以为删掉了"当成事实。
                  const after = await readMemoryBackendState({
                    settings: currentSettings(), env: process.env, workspace: process.cwd(),
                    mcpClientInstalled: mcpClientInstalled(), hindsight: await hindsightSummary(),
                  });
                  json(200, { ...removed, ok: true, state: after, settings: currentSettings() });
                  return;
                }
                const backend = String((body && body.backend) || '').trim();
                const out = await applyMemoryBackend({ backend, env: process.env, workspace: process.cwd() });
                if (!out.ok) {
                  json(out.status || 400, { ok: false, path: out.path, backend: out.backend, errors: out.errors, notes: out.notes });
                  return;
                }
                // 上面已经拒了非法值 ⇒ 这里存的一定是 spec 认的值（设置项真源仍是 `lib/settings.js`）。
                const written = await persistSettingsPatch({ 'memory.backend': out.backend });
                if (!written.ok) {
                  // 两处存储没法做成一个事务 ⇒ 半成功必须**明说**（否则设置页会显示"设置与实际不一致"却没人解释）。
                  json(written.status, {
                    ok: false, errors: written.errors, settings: written.settings, memoryBackend: out,
                    note: `profile 补丁已按「${out.backend}」落地，但**设置值没存上** ⇒ 设置页会显示"设置与实际不一致"。请重试一次。`,
                  });
                  return;
                }
                const state = await readMemoryBackendState({
                  settings: written.settings, env: process.env, workspace: process.cwd(), mcpClientInstalled: mcpClientInstalled(),
                  hindsight: await hindsightSummary(),
                });
                json(200, { ...out, ok: true, settings: written.settings, settingsSource: written.viaHost ? 'host' : 'file', state, midasSetup: state.midasSetup });
                return;
              }
              // `?probe=1`：**真的启动一次** Midas 并做 MCP 握手 —— 只有用户点了才传（进页面不 spawn 任何进程）。
              const wantProbe = String(query.get('probe') ?? '') === '1';
              // GET 用**当前设置**读实际状态（`memory.backend` 的真实消费者就在这一行）——
              // 这一句的写法被 `memory-backend.test.mjs` ⑦ 的接线断言逐字钉着（别顺手改形状）。
              const state = await readMemoryBackendState({ settings: currentSettings(), env: process.env, workspace: process.cwd(),
                mcpClientInstalled: mcpClientInstalled(), midasProbe: wantProbe, hindsight: await hindsightSummary() });
              json(200, {
                ok: true,
                ...state,
                // 中文标签**随响应到达客户端**：客户端是手写 bundle、不能 `import lib/`，标签只能这样过去。
                // 真源仍是 `lib/settings.js` 的 spec（`MEMORY_BACKENDS` 与它集合相等，有测试钉住），这里只是照抄。
                labels: (SETTINGS_SPEC.memory && SETTINGS_SPEC.memory.items.backend && SETTINGS_SPEC.memory.items.backend.labels) || null,
                // Midas 的**五态结论 + 首装引导**（步骤里的命令也由服务端给，客户端不硬编）：显式带出来，
                // 让客户端契约一眼可见（`state` 里本来也有，这里是同一次请求的**同一份**对象，不是第二份）。
                midasSetup: state.midasSetup,
                // 探测失败/成功都要能说清"这是刚探的"：把是否真探过也带出去（不猜）。
                midasProbed: wantProbe === true && !!(state.midasSetup && state.midasSetup.start),
              });
            }, { methods: ['GET', 'POST'] }),
          });
          wctx.effect(() => disposeMemoryBackend);
