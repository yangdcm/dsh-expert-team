...
> 机读真源是 `lib/vocab.js` 的 `ROLE_LABELS_ZH`（**条目与取值以它为准，本表不另立定义**）：
...
| `dba` | 数据工程师 | `sec` | 安全审计员 |
| `ui-verifier` | UI 验证专家 | `debugger` | 故障诊断工程师 |
...
- **编排者**：用随技能分发的脚本 `node <skill-dir>/scripts/scan-single-source.mjs <事实名> --root <项目根>`（`<skill-dir>` = 本包技能目录，脚本随技能一起分发）列出**全部**定义点与引用点；多处定义写法分叉时脚本会直接报警。扫完在 `RUN.log.md` 追加 `scan:single-source — <事实名> · 命中 N 处`。
...
4) **「沉默清单」是必产出（第五道验证）**：现有三道验证（代码 vs 规格 / 规格 vs 自身一致性 / 契约 vs 实现）**全部默认规格是对的**。所以 spec-review 阶段必须额外列出「**规格未规定、但实现或交互上可选的行为**」，逐条给出「建议裁定（允许 / 禁止）+ 依据 + 风险」，交 PM/用户裁定。**产出落在 `REVIEW-SPEC.md`**（**定名**：spec 审查产出 `REVIEW-SPEC.md`、代码审查产出 `REVIEW.md`，两者都归 `reviewer`）。**沉默不得作为通过理由**（实证：规格沉默 ⇒ 自回复被实现，且无人报 finding）。
