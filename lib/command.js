...
import {
  ...
  LAY_TMP_PREFIX, LAY_STATES, LAY_ACTIONS, LAY_KINDS,
} from './preset-lay.js';
...
async function layInstalledAsset(kind, srcDir, target, opts = {}) {
  ...
  if (force) {
    ...
    if (plan.action === 'skip-fresh' && opts.forceEvenFresh !== true) {
      recordLayOutcome(kind, { ...plan, target, version: PLUGIN_VERSION });
      return target;
    }
    ...
    recordLayOutcome(kind, { ...plan, target, version: '' });
...
async function listSubagentStatusBySession(ctx, sid, knownIds, opts) {
  ...
  // 每次调用先复位 —— `notReady` 是**本次**的事实，不许粘住（粘住就等于又造了一盏常亮灯）。
  SUB_HEADER_STATS.notReady = false;
  ...
  if (root && ids.length) {
    ...
    if (memoNow && memoNow.provisional && !covered) {
      SUB_HEADER_STATS.cut = SUB_HEADER_STATS.cut || 'warming';
      // `notReady`（2026-09-16）：这一态过去**只**写进内部 `cut`，没有任何字段进响应 ⇒
      // 真机重启后首轮 `agents=[] / subsPending=13` 与"这个团队真的没有人"在响应里
      // **一模一样**（本仓纪律："两种零分不开"）。现在把它变成一个**明确的**事实，
      // 由调用方挂 `warming:['subs']`（沿用既有语义，不造新概念）。
      // 限时 `SUBS_NOT_READY_MAX_MS`：一直拿不到就把"还没就绪"这句收回，交给 `subsPending`
      // 如实表达"明细拿不到" —— 否则它自己又会变成一盏常亮的灯。
      const since = Number(memoNow.provisionalSince) || 0;
      SUB_HEADER_STATS.notReady = !!since && (Date.now() - since) <= SUBS_NOT_READY_MAX_MS;
    }
  }
